...
class AdvertisingManager {
    ...
    {
        publishers, ads, availableBanners := adService.startPublish(publisher, rentPrice, publishers, ads, availableBanners);
    }
...
class AdvertisingService
{
    ...
    {
        var newAd : Ad := new Ad(banner, rentPrice);
    ...
    ensures minPublisherFound(retPrice, retPublisher, publishers, ads)
    {
        ...
        {
            var curPrice := ads[publishers[index]].rentPrice;
    ...
    ensures startPublishInvariant(publisher, rentPrice, publishers, retPublishers, ads, retAds, availableBanners, retBanners, globalMinPublisher)
    {
        ...
        if (|availableBanners| > 0)
        {
            ...
            newAds := addAd(publisher, rentPrice, newBanners[0], newAds);
        ...
        else
        {
            if (|newPublishers| > 0)
            {
                ...
                if (rentPrice > minPrice)
                {
                    var banner := newAds[minPublisher].banner;
                    newAds := addAd(publisher, rentPrice, banner, newAds);
